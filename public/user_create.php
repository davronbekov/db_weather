...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $scope = $_POST["scope"];
    $email = $_POST["email"];
    $response = DB::query("INSERT INTO `User` (email, login, password, scope) VALUES (?,?,?,?)",
        [$email, $login, $password, $scope]);
    if ($response->errorCode() != '00000') {
        print_r($response->errorInfo());
        die();
    } else {
        header("Location: /db_weather/public/index.php");
    }
}
?>
<!doctype html>
  ...
  <div class="app" id="app">
    ...
    <article class="content item-editor-page">
      <div class="title-block">
        <h3 class="title"> Add new user <span class="sparkline bar" data-type="bar"></span>
        </h3>
      </div>
      <form name="item" action="user_create.php" method="post">
        <div class="card card-block">
          <div class="form-group row justify-content-center">
            <label class="col-sm-2 form-control-label text-xs-right"> Login: </label>
            <div class="col-sm-5">
              <input type="text" class="form-control boxed" name="login" placeholder="" required>
          ...
          <div class="form-group row justify-content-center">
            ...
            <div class="col-sm-5">
              <input type="email" class="form-control boxed" name="email" placeholder="" required>
            </div>
          </div>
          <div class="form-group row justify-content-center">
            <label class="col-sm-2 form-control-label text-xs-right"> Password: </label>
            <div class="col-sm-5">
              <input type="password" class="form-control boxed" name="password" placeholder="" required>
            </div>
          </div>
          <div class="form-group row justify-content-center">
            <label class="col-sm-2 form-control-label text-xs-right"> Scope: </label>
            <div class="col-sm-5">
              <input type="text" class="form-control boxed" name="scope" placeholder="" required>
            </div>
          </div>
          <div class="form-group row justify-content-center">
...
<script>
  ...
  $(function () {
    ...
    input.on('input', function (event) {
      fetch("/db_weather/public/user_exist.php?login=" + input.val(), {})
        .then(function (data) {
          return data.json();
        }).then(function (data) {
        if (data.status === 'exist') {
          ...
          input.siblings('.invalid-feedback').html("User already exist");
          $('[type=submit]').prop( "disabled", false );

        } else {
          input.removeClass('is-invalid')
          input.addClass('is-valid');
          input.siblings('.valid-feedback').html("User is unique");
